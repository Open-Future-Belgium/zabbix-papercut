...
>Steps: Install latest python 3.x; create folder "c:\zabbix"; place "zabbix-papercut-linux-py2.zip" and "zabbix-3.4.3.tar.gz" in there.
Run this script
'''
import os
import shutil
...
def PcZipCopy():
    ...
    zip_ref.close()            
    for file in glob.glob(r'tmpz\zabbix-papercut-master\linux\py2\*.py'):
...
# Edit /binz/papercut.conf file
    ...
    hmu = raw_input('Enter PaperCut System Health Monitoring URL: ').strip()
    m = re.search(p,hmu)
    pcip = m.group('host') # 'www.abc.com'
    pcAuth = m.group('auth') # '123'
    print (pcip)
    print (pcAuth)
#    pcip = raw_input('Enter PaperCut server IP address [127.0.0.1] : ').strip()
    if not pcip:
        pcip = '127.0.0.1'     
#    pcAuth = raw_input('Enter Authorization code : ').strip()
    if not pcAuth:
...
#Edit binz/zabbix_agent.d.conf file
    zxip = raw_input('Enter Zabbix Server IP address [127.0.0.1] : ').strip()
    if not zxip:
        zxip = '127.0.0.1'
        szxip = 'Server='+ zxip     
    else:
        szxip = 'Server='+ zxip        
    zxaip = raw_input('Enter Zabbix active Server IP address [] : ').strip()
    if not zxaip:
